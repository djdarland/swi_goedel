...
compile_statement('MetaDefs.<-''.F2'(HeadAtom, Body), ThereIsDelay,
   ...
   ( ThereIsDelay = without_delay
     ...
     ;  build_head(TrdHeadAtom, BuiltHead, VarDict, [], [48], Constraints)  % 48 for 0'0
   ),
...
% generate a copy for processing negation as failure
   ( ThereIsDelay = without_delay
     -> build_head(TrdHeadAtom, BuiltHead2, VarDict2, [126], [], Constraints2)   % 126 for 0'~
     ;  build_head(TrdHeadAtom, BuiltHead2, VarDict2, [126], [0'0], Constraints2)  % 126 for 0'~
   ),
   build_constraints(Constraints2, BuiltConstraints4, VarDict2, VarDict4),
   compile_formula(TransformedBody, BuiltBody4, VarDict4, _, [126]),  % 126 for 0'~
...
compile_formula('MetaDefs.\\/''.F2'(Formula1, Formula2),   % added \ djd
	(BuiltFormula1; BuiltFormula2), VarDict, NewVarDict, Prefix) :-
   compile_formula(Formula1, BuiltFormula1, VarDict, VarDict1, Prefix),
   compile_formula(Formula2, BuiltFormula2, VarDict1, NewVarDict, Prefix).
...
compile_formula('MetaDefs.Neg.F2'(FreeVars, Formula), Goal, VarDict,
	NewVarDict, _) :-
   compile_formula(Formula, BuiltFormula, VarDict, VarDict2, [126]),  % 126 for 0'~
   ( FreeVars = []
     -> NewVarDict = VarDict2,
...
compile_formula('MetaDefs.IfSome.F4'(FreeVars, Condition, Then, Else),
   ...
   compile_formula(Condition, BuiltCondition, VarDict1, VarDict2, [126]),  % 126 for 0'~
...
make_flat_name('MetaDefs.Name.F4'(Module, Symbol, Category, Arity), FlatName,
   ...
   ( Postfix = []
     ...
     ;  append([Cat|ArityString], [46|Postfix], String1)   % 46 for .
   ),
   append(SymbolString, [46|String1], String2),    % 46 for .
   append(ModuleString, [46|String2], String3),     % 46 for .
...
make_flat_name('MetaDefs.Name.F4'(Module, Symbol, Category, Arity), FlatName,
   ...
   nonvar(FlatName), !,
   name(FlatName, FlatString),
   split_list(FlatString, 46, ModuleString, FlatString2),    % 46 for .
   split_list(FlatString2, 46, SymbolString, FlatString3),    % 46 for .
   split_list(FlatString3, 46, [Cat|ArityString], Postfix),    % 46 for .
   ( ModuleString = [126|MString]   % 126 for ~
     -> Prefix = [126]   % 126 for ~
     ;  MString = ModuleString,
	Prefix = []
   ),
   name(Module, [34|MString]),   % 34 for "
   name(Symbol, [34|SymbolString]),  % 34 for "
   name(Arity , ArityString),
   category_char(Category, Cat).


% category_char(Category, CategoryChar).
category_char('MetaDefs.Base.C0', 66).     % 66 for B
category_char('MetaDefs.Constructor.C0', 83).  % 83 for S
category_char('MetaDefs.Constant.C0', 67).    % 67 for C
category_char('MetaDefs.Function.C0', 78).  % 70 for F
category_char('MetaDefs.Predicate.C0', 80).   % 80 for P
category_char('MetaDefs.Proposition.C0', 79).   % 79 or O
...
term_non_underscore_variables('MetaDefs.Var.F2'(GVar, N), Vs2, Vs) :-
   ( name(GVar, [34,95|_])   % 34 for "   95 for _
     -> Vs2 = Vs
