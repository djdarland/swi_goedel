   ...
   ( NewCondition = []
	...
	( Layer = 0
	  -> Postfix = []
        ...
        generate_delay_clauses(Condition, Name, Argus, Postfix, [48],  % 48 for 0
		ModuleName)
...
generate_delay_clauses_normal(Condition, Name, Argus, Postfix1, Postfix2,
   ...
   make_flat_name(Name, NegPredicate1, [126], Postfix1),  % 126 for ~
   NegHead =.. [NegPredicate1|Argus],
   make_flat_name(Name, NegPredicate2, [126], Postfix2),  % 126 for ~
...
handle_special_delay_aux([Predicate/Arity|List]) :-
   ...
   make_flat_name(Name, Predicate2, [], [48]),  % 48 for 0
   Body =.. [Predicate2|VarArgus],
   portray_clause( (Head :- Body) ),

   make_flat_name(Name, Predicate3, [126], Postfix),  % 126 for ~
   NegHead =.. [Predicate3|VarArgus],
   make_flat_name(Name, Predicate4, [126], [48]),  % 126 for ~ 48 for 0
