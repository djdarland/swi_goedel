:- module(parser,[parse_program/3]).  %% converted djd
...
:- discontiguous language_decl1/8. %% added djd
:- discontiguous language_decl2/8. %% added djd
...
parse_program(ModuleName, Switch, Program):-
    trace(module_as_items),
    trace(string2Gstring),
    trace(identify_module_kind),
    trace(identify_module_part),
    trace('ParserPrograms.QuickNewProgram.P3'),
    trace(parse_module_decl),
    trace('ParserPrograms.ExportSymbolTable.P3'),
    trace(parse_statements),
    trace(parse_statements_aux),

    trace(statement),
%    trace(item),
    trace(print_error_return),
    trace('SharedPrograms.ProgramLanguage.P2'),
    trace('ParserPrograms.ParserCheckStatement.P4'),
    trace(get_predicate_arity),
    trace('ParserPrograms.QuickInsertStatement.P6'),
    trace(singleton_variable_checking),
    trace(quantified_variable_checking),
    trace(floundering_checking),
    trace(print_type_error),

   module_as_items(ModuleName, ExpTokens, LocTokens),
   write("DJD Module name = "),
   write(ModuleName),nl,
   string2Gstring(ModuleName, GModuleName),
   write("DJD Module name BBB = "),
   write(ModuleName),nl,
...
or_seq([Token|Tokens], RestTokens, Condition, WholeTokens, Error, Ln1, Ln2,
   ...
   cond1(Token, Tokens, RestTokens2, Cond1, WholeTokens, Error, Ln1, Ln2,
		WhichPart),
   ( var(Error)
     -> ( RestTokens2 = [graphic_name('\\/')|RestTokens3] % added \ djd
	  -> Condition = 'ProgDefs.Or.F2'(Cond1, Cond2),
...
parse_statements_aux([Item|Items], ModuleName, Language, OldProg, NewProg):-
   ...
   write("DJD parse_sttements_aux - about to recurse"),nl,
...
print_out_error(Format, Parameters, WhichPart, Ln1, Ln2, Tokens, Position) :-
   name(Format, Chars),
   ( Chars = [69, 114, 114, 111, 114, 58 | Rest]  % 69 for E 111 for o 114 for r  58 for :
     -> assert(there_is_error),		% message to stop compilation
	format(user_error, '~nError:', [])
     ;  ( Chars = [87, 97, 114, 110, 105, 110, 103, 58 | Rest] % 87 for W 97 for a 114 for r 110 for n 105 for i 110 for n 103 for g 58 for : 
