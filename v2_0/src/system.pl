...
'ParserPrograms.AddLiftedSymbols.P3'(['ProgDefs.Lift.F2'(B,C)|A], D, E) :-
        (   'lists_g.Member.P2'('ParserPrograms.ImportItem.F3'(B,_,_), D) ->
            F=D
        ;   'lists_g.DeleteFirst.P3'('ParserPrograms.LiftedItem.F2'(B,G), D, H) ->
...
'ParserPrograms.LocalSymbolTable.P3'('ProgDefs.Program.F4'(_,A,'ProgDefs.Language.F1'(B),_), C, 'ParserPrograms.SymbTab.F4'(C,D,E,[])) :-
        ...
        'lists_g.Append.P3'(H, I, K),
        'lists_g.Append.P3'(J, K, L),
...
'ParserPrograms.ImportedSymbols.P5'([A|B], C, D, E, F) :-
        (   'lists_g.Member.P2'('ParserPrograms.ImportItem.F3'(A,_,_), E) ->
            G=E,
            H=B
        ;   (   'lists_g.DeleteFirst.P3'('ParserPrograms.LiftedItem.F2'(A,_), E, I) ->
                J=I
            ;   J=E
            ),
...
'ParserPrograms.QuickInsertDelay.P7'('ProgDefs.Program.F4'(A,B,C,D), E, F, G, H, I, 'ProgDefs.Program.F4'(A,B,C,J)) :-
        ...
        (   'lists_g.DeleteFirst.P3'('ProgDefs.PredDef.F4'(G,P,Q,R), O, S) ->
...
'ParserPrograms.QuickNewProgram.P3'(A, B, C) :-
    write("DJD system quicknewprogram\n"),
    write("A = "),write(A),nl,
    write("B = "),write(B),nl,
    write("C = "),write(C),nl,
        'SharedPrograms.ProgramSkeleton.P4'(A, B, 0, C),
    write("DJD BBB system quicknewprogram\n"),
    write("A = "),write(A),nl,
    write("B = "),write(B),nl,
    write("C = "),write(C),nl.
...
'ParserPrograms.QuickInsertStatement.P6'('ProgDefs.Program.F4'(A,B,C,D), E, F, G, H, 'ProgDefs.Program.F4'(A,B,C,I)) :-
        ...
        (   'lists_g.DeleteFirst.P3'('ProgDefs.PredDef.F4'(G,O,P,Q), N, R) ->
...
'ParserPrograms.UpdateLiftedSymbols.P5'([A|B], C, D, E, F) :-
        (   'lists_g.DeleteFirst.P3'('ProgDefs.Lift.F2'(A,G), E, H) ->
            I=G,
            J=H
        ;   J=E,
...
'ParserPrograms.UpdateImportedSymbols.P4'([A|B], C, D, E) :-
        'lists_g.MemberCheck.P2'('ProgDefs.Lift.F2'(A,F), C),
        (   'lists_g.DeleteFirst.P3'('ParserPrograms.LiftedItem.F2'(A,_), D, G) ->
            H=['ParserPrograms.LiftedItem.F2'(A,F)|G]
        ;   'lists_g.MemberCheck.P2'('ParserPrograms.ImportItem.F3'(A,_,_), D) ->
            H=D
...
'ParserPrograms.VariantTypelists_g.P2'(ListA, ListB) :-
...
'SharedPrograms.AddOtherModules.P7'('ProgDefs.ModDef.F4'(_,A,B,C), D, E, F, G, H, I) :-
        'lists_g.Append.P3'(C, A, J),
        'lists_g.Append.P3'(B, J, K),
...
'SharedPrograms.ExpandStringAux.P3'([A|B], C, D) :-
        (   A=34 ->
            'SharedPrograms.CharDL.P3'('"\\', C, E),  % added \ djd
            'SharedPrograms.CharDL.P3'('""', E, F)
        ;   A=92 ->
            'SharedPrograms.CharDL.P3'('"\\', C, E),  % added \ djd
            'SharedPrograms.CharDL.P3'('"\\', E, F)  % added \ djd
        ;   C=[A|F]
        ),
...
'SharedPrograms.ModuleLanguage.P3'('ProgDefs.Program.F4'(_,A,'ProgDefs.Language.F1'(B),_), C, 'ProgDefs.Language.F1'(D)) :-
        ...
        ;   (   'avlTrees.avlSearch.P3'(B, C, J) ->
            ...
            'lists_g.Append.P3'(G, H, L),
            'lists_g.Append.P3'(I, L, M),
...
'SharedPrograms.OrSeqToIntDL.P3'('ProgDefs.Or.F2'(A,B), C, D) :-
        'SharedPrograms.SimpleCondToIntDL.P3'(A, C, E),
        'Strings.StringInts.P2'('" \\/ ', F), % added \ djd
        'lists_g.Append.P3'(F, G, E),
...
'SharedPrograms.FindMember.P2'(X, Ys) :-
   ( var(X) ->
     Ys = [Z|Zs],
     member_in(Zs, Z, X)
   ; user:'lists_g.MemberCheck.P2'(X, Ys)
   ).
...
'SharedPrograms.PickOne.P3'(X, Ys, Rest) :-
   ...
   ; user:'lists_g.DeleteFirst.P3'(X, Ys, Rest)
   ).
...
'SharedPrograms.FindPredDef.P3'(Arity, PredDefs
   ...
   ( var(Arity) ->
     user:'lists_g.Member.P2'('ProgDefs.PredDef.F4'(Arity, Statements
           , Delays1, Delays2)
        , PredDefs)
   ; user:'lists_g.MemberCheck.P2'('ProgDefs.PredDef.F4'(Arity, Statements
           , Delays1, Delays2)
        , PredDefs)
   ).
...
'SharedPrograms.PickPredDef.P4'(Arity, PredDefs
   ...
   ( var(Arity) ->
     'lists_g':'lists_g.Delete.P3'('ProgDefs.PredDef.F4'(Arity, Statements
           , Delays1, Delays2)
        , PredDefs, RestDefs)
   ; 'lists_g':'lists_g.DeleteFirst.P3'('ProgDefs.PredDef.F4'(Arity, Statements
           , Delays1, Delays2)
        , PredDefs, RestDefs)
   ).
...
'Substs.ComposeTypeSubsts2.P3'('Substs.TypeSubst.F2'(A,B), 'Substs.TypeSubst.F2'(C,D), 'Substs.TypeSubst.F2'(E,F)) :-
        ...
        'Substs.Composelists_g.P3'(D, B, F).
...
'Substs.Composelists_g.P3'([], A, A).
'Substs.Composelists_g.P3'(['MetaDefs.!.F2'(B,C)|A], D, ['MetaDefs.!.F2'(B,C)|E]) :-
        'Substs.DB.P3'(D, B, F),
        'Substs.Composelists_g.P3'(A, F, E).
...
'Substs.DelTypeBinding.P4'('MetaDefs.Par.F2'(A,B), C, 'Substs.TypeSubst.F2'(D,E), 'Substs.TypeSubst.F2'(D,F)) :-
        'lists_g.DeleteFirst.P3'('MetaDefs.!.F2'('MetaDefs.Par.F2'(A,B),G), E, F),
...
'Substs.DelTermBinding.P4'('MetaDefs.Var.F2'(A,B), C, 'Substs.TermSubst.F2'(D,E), 'Substs.TermSubst.F2'(D,F)) :-
        'lists_g.DeleteFirst.P3'('MetaDefs.!.F2'('MetaDefs.Var.F2'(A,B),G), E, F),
...
'Substs.DerefType.P3'('MetaDefs.Par.F2'(A,B), 'Substs.TypeSubst.F2'(C,D), E) :-
        user:one_solution(('lists_g.MemberCheck.P2'('MetaDefs.!.F2'('MetaDefs.Par.F2'(A,B),F),D)->'Substs.DerefType.P3'(F,'Substs.TypeSubst.F2'(C,D),E);E='MetaDefs.Par.F2'(A,B))).
...
'Substs.Dereference.P3'('MetaDefs.Var.F2'(A,B), 'Substs.TermSubst.F2'(C,D), E) :-
        user:one_solution(('lists_g.MemberCheck.P2'('MetaDefs.!.F2'('MetaDefs.Var.F2'(A,B),F),D)->'Substs.Dereference.P3'(F,'Substs.TermSubst.F2'(C,D),E);E='MetaDefs.Var.F2'(A,B))).
...
'Substs.ParameterInSubst.P3'('Substs.TypeSubst.F2'(_,A), B, C) :-
        'lists_g.Member.P2'('MetaDefs.!.F2'(B,C), A).
