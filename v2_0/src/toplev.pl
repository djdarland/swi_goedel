:- module(toplev, [top_loop/3,no_message/0,checking_is_off/0,next_command/3,process_command/7,build_command/7]).   % converted djd
...
next_command(ModuleName, Prog, SymbolTable) :-
   ...
   write("DJD top_djd_next_command 00004\n"),
   !,
   write("DJD top_djd_next_command 00005\n"),
   ...
   write("DJD top_djd_next_command 00006\n"),
...
make_prompts_orig(Module, Prompt1, Prompt2) :-
    write("Dgb djd top make_prompts 0001\n"),
   name(Module, String1),
    write("Dgb djd top make_prompts 0002\n"),
...
%% globally replaced ttyget0 by get_code djd
...
read_to_terminator(46, C2, _, [46], not_in_string) :-    % DJD for .
   blank_char(C2), !,
   myttyskip(C2, 10).

read_to_terminator(37, C2, _, [46], not_in_string) :-   % DJD % .
   !,
   myttyskip(C2, 10).

read_to_terminator(92, C2, Prompt, [92, C2|Cs], in_string) :-  !, % added 2 \ DJD  --  DJD for \
   get_one_char(C2, Prompt, C3, in_string),
   get_one_char(C3, Prompt, C4, in_string),
   read_to_terminator(C3, C4, Prompt, Cs, in_string).

read_to_terminator(34, C2, Prompt, [34|Cs], in_string) :-  !,   % for " DJD
...
get_one_char(46, Prompt, C, Switch) :- !,   % for . djd
   get_code(C),
...
remove_true(Goal ' & ' true, GGoal) :- !,
   remove_true(Goal, GGoal).
remove_true(Goal ' \\/ ' true, GGoal) :- !,  % added \ djd
   remove_true(Goal, GGoal).
remove_true(true ' \\/ ' Goal, GGoal) :- !,  % added \ djd
   remove_true(Goal, GGoal).
remove_true('~ ' Goal, '~ ' GGoal) :- !,
   remove_true(Goal, GGoal).
...
convert_to_goedel_formula((Formula1; Formula2), GFormula) :-
   ...
   ( Formula1 = (Cond -> _Then)
     -> GFormula = ('IF' GCond 'THEN ... ELSE ...'),
        convert_to_goedel_formula(Cond, GCond)
     ;  GFormula = (GFormula1 ' \\/ ' GFormula2),  % added \ djd
        convert_to_goedel_formula(Formula1, GFormula1)
   ).
...
convert_to_goedel_formula(difference(Term1, Term2, Term3),
				'='(GTerm3, '\\'(GTerm1, GTerm2))) :-  % added \ djd
   convert_set_terms([Term1, Term2, Term3], [GTerm1, GTerm2, GTerm3]).
...
constraint_goal_conversion(difference, 'Sets', '\\', 2).  % added \ djd
...
gstring(String) :- atom(String), name(String, [34|_]).   % for " djd
