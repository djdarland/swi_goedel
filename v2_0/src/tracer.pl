...
tracer_question_aux(98, _, _, _) :- !,  % 98 for b
   break.

tracer_question_aux(97, _, _, _) :- !,  % 97 for a
   raise_exception(catch_in_query).

tracer_question_aux(115, N, call, _) :- !,  % 115 for s
   assert(skip_trace(N)).

tracer_question_aux(114, _, fail, redo) :- !.  % 114 for r

tracer_question_aux(102, _, _, _) :- !,  % 102 for f
   assert(free_trace).

tracer_question_aux(108, _, _, _) :- !,  % 108 for l
   retractall(start_trace),
   assert(stop_trace).

tracer_question_aux(99, _, _, _) :- !,   % 99 for c 
   assert(no_trace).

tracer_question_aux(110, _, _, _) :- !.   % 110 for n

tracer_question_aux(10, _, _, _) :- !.

tracer_question_aux(63, _, _, _) :- !,   % 63 for ?
   display_tracer_message.

tracer_question_aux(104, _, _, _) :- !,   % 104 for h
   display_tracer_message.
